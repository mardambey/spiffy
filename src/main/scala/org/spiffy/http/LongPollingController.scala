...
trait LongPollingController extends Actor
{
  ...
  def receive = {        
	...
	case ignore => {
	  log.debug("Client did not provide sessionKey or sessionKey not registered, consider handshaling first: " + s.ctx + " - " + s.req.getSession)
	  s.ctx.complete
	}	
      }      
    }
	...
	case Some(key) if (LongPollingController.sessions.contains(key)) => {
	  onDataReceived(s)
	  LongPollingController.send(s, "{data:\"foobar\"}")
    ...
    case s @ R(BASE, "handshake") => {
      ...
      log.debug("Handshake: " + sessionKey + " - " + resp)
